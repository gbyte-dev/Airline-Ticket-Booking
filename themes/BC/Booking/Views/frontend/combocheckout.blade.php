                    ...
                    <div class="col-md-12 col-12">
                        <div class="card-header text-center ">
                            <b class="font_manish " style="color: #ff7d01">Departure
                                Flight</b>
                            {{-- <h4 class="float-right depature_web_font">Total Price:<span style="color: #ff7d01"> <?php //= $icon
                            ?>{{ number_format($combo['totalFare'], 2)}} </span></h4> --}}
                        </div>
                    </div>
                    <div class="col-md-12 col-12">
                        <span class="card-title"> {{ $combo['departureLocation'] ?? '' }} →<span class='font_manish';>
                                {{ $combo['triptimeReq'] ?? '' }} </span>→ {{ $combo['city_aa'] ?? '' }}
                            ({{ $combo['city_code'] ?? '' }})</span>
                        <span class="float-right font_manish " style="margin-left: 34%;">
                            @if (isset($combo['departureDate']))
                                {{ date('D, d M Y', strtotime($combo['departureDate'])) }}
                            @endif
                        </span>
                    </div>
                    @foreach ($combo['segment2'] as $seg)
                        <hr>
                        @php
                            $id = $seg['id'] ?? '';
                            ...
                            $stopNumber = $seg['sN'] ?? '';
                        @endphp
                        @if (isset($departureCode) && $departureCode == $combo['to'])
                            <div class="row">
                                ...
                                <div class="col-sm-12 col-12">
                                    <span class="card-title"> {{ $combo['city_aa'] ?? '' }}
                                        ({{ $combo['city_code'] ?? '' }})
                                        →<span class='font_manish';>{{ $combo['triptimeReturn'] ?? '' }} </span> →
                                        {{ $combo['departureLocation'] ?? '' }}</span>
                                    <span
                                        class="float-right font_manish ">{{ date('D, d M Y', strtotime($departureDate)) }}</span>
                                </div>
                            </div>
                        ...
                        <table class="table">
                            <tbody>
                                <tr>
                                    ...
                                    <td class="text-right">{{ $combo['iB'] > 0 ? $combo['iB'] . ' /Adult' : 'No' }}</td>
                                </tr>
                                <tr>
                                    <td> Cabin baggage :</td>
                                    <td class="text-right">{{ $combo['cB'] > 0 ? $combo['cB'] . ' /Adult' : 'No' }}</td>
                                </tr>
                            </tbody>
                        </table>
                    </div>
                        ...
                        <table class="table">
                            <tbody>
                                ...
                                <tr>
                                    ...
                                    <td class="text-right">{!! $icon !!}{{ number_format($Refamount + $additionalFee + $agency, 2) }}
                                        <div class="icon-container">
                                            ...
                                            <span class="hover-text">
                                                <table>
                                                    <tr>
                                                        <td>Flight Cost</td>
                                                        <td class="text-right">{!! $icon !!}{{ number_format($combo['totalFare'], 2) }}
                                                        <td>
                                                    </tr>
                                                    <tr>
                                                        <td>Airline Fee</td>
                                                        <td class="text-right">{!! $icon !!}{{ number_format($Refamount + $additionalFee, 2) }}</td>
                                                    </tr>
                                                    <tr>
                                                        <td>Agency Fee</td>
                                                        <td class="text-right">{!! $icon !!}{{ number_format($agency, 2) }}</td>
                                                    </tr>
                                                </table>
                                            </span>
                                        </div>
                            ...
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        ...
                                        <td class="text-right">{!! $icon !!}{{ number_format($totalDateRefund, 2) }}+Fare
                        ...
                        <div class="card-body">
                            <table class="table">
                                <tbody>
                                    ...
                                    <tr>
                                        <td>Agency Fee</td>
                                        <td>{!! $icon !!}{{convertCurrency(400, $curr_currency) }}</td>
                                    </tr>
                                </tbody>
                            </table>
                        </div>
                        ...
                        <div class="table-responsive">
                            <table class="table table-striped table-bordered m-0">
                                <tr>
                                    ...
                                    <td class="text-right"><?= $icon?>{{ number_format($combo['totalBaseFares'], 2) }}</td>
                                </tr>
                                <tr>
                                    <td class="text-right">Tax</td>
                                    <td class="text-right"><?= $icon?>{{ number_format($combo['totalTaxess'], 2) }}
                                    </td>
                                </tr>
                                <tr>
                                    <th class="text-right">Total Fare</th>
                                    <th class="text-right"><?= $icon?>{{ number_format($combo['totalFare'], 2) }}
                                    </th>
                                </tr>
                            </table>
                        </div>
...
<div class="virat" style="display: none">
    <form action="{{ route('booking.flightBooking') }}" method="post" id="form-checkout-threedata">
        @csrf
        <ul>
            <input type="hidden" name="airCode" value="{{ $combo['airCode'] }}" />
            <input type="hidden" name="code" value="{{ $combo['code'] }}" />
            <input type="hidden" name="airName" value="{{ $combo['airName'] }}" />
            <input type="hidden" name="flightNumbers" value="{{ $combo['flightNumbers'] }}" />
            <input type="hidden" name="departureTimes" value="{{ $combo['departureTimes'] }}" />
            <input type="hidden" name="arrivalTimes" value="{{ $combo['arrivalTimes'] }}" />
            <input type="hidden" name="durationText" value="{{ $combo['durationText'] }}" />
            <input type="hidden" id="departureDate" name="on_date" value="{{ $combo['departureDate'] }}" />
            <input type="hidden" name="stopText" value="{{ $combo['stopText'] }}" />
            <input type="hidden" id="departureLocation" name="departureLocation"
                value="{{ $combo['departureLocation'] }}" />
            <input type="hidden" id="arrivalLocation" name="arrivalLocation" value="{{ $combo['arrivalLocation'] }}" />
            <input type="hidden" name="adults" value="{{ $combo['adults'] }}" />
            <input type="hidden" name="children" value="{{ $combo['children'] }}" />
            <input type="hidden" name="infants" value="{{ $combo['infants'] }}" />
            <input type="hidden" name="currency_type" value="{{ $icon }}" />
            <input type="hidden" name="totalBaseFare" value="{{ $combo['totalBaseFare'] }}" />
            <input type="hidden" name="totalTaxes" value="{{ $combo['totalTaxes'] }}" />
            <input type="hidden" name="totalDiscount" value="{{ $combo['totalDiscount'] }}" />
            <input type="hidden" name="totalFare" value="{{ $combo['totalFare'] }}" />
            <input type="hidden" name="arrivalDate" value="{{ $combo['arrivalDate'] }}" />
            <input type="hidden" name="flight_details" value="{{ json_encode($combo['segment2']) ?? '' }}" />
            <input type="hidden" name="flight_class"
                value="{{ isset($combo['flight_class']) ? $combo['flight_class'] : '' }}" />
            {{-- <input type="hidden" name="from" id="departureLocation" value="{{$getdata[1]['departureLocation']?$getdata[1]['departureLocation']:''}}">
                <input type="hidden" name="to" id="arrivalLocation" value="{{$getdata[1]['arrivalLocation']?$getdata[1]['arrivalLocation']:''}}">
                <input type="hidden" name="on_date" id="departureDate" value="{{$getdata[1]['departureDate']?$getdata[1]['departureDate']:''}}">
                <input type="hidden" name="return_date" id="returndate" value="{{$returndata[1]['departureDate']?$returndata[1]['departureDate']:''}}"> --}}
            <input type="hidden" name="flight_type" id="flight_type"
                value="{{ $combo['departureDate'] ? 'return' : 'One way' }}">
            <input type="hidden" name="random_id" value="{{ rand(0000, 999999999999999999) }}" />
            @if (isset($combo['returnDate']) && $combo['returnDate'] != '')
                <input type="hidden" name="from" value="{{ $combo['from'] ?? '' }}" />
                <input type="hidden" name="to" value="{{ $combo['to'] ?? '' }}" />
                <input type="hidden" name="date" value="{{ $combo['date'] ?? '' }}" />
                <input type="hidden" id="returndate" name="return_date"
                    value="{{ $combo['returnDate'] ?? '' }}" />
            @endif
            @if (is_array($combo['stopsTime']))
                <input type="hidden" name="stopsTime" value="{{ json_encode($combo['stopsTime']) }}" />
            @else
                <input type="hidden" name="stopsTime" value="{{ $combo['stopsTime'] }}" />
            @endif
        </ul>
        {{-- @dd($combo) --}}
                                                                ...
                                                                <div class="form-section">
                                                                    ...
                                                                    <div class="form-group" style="display: none">
                                                                        ...
                                                                        <select id="numAdults" class="form-select"
                                                                            ...
                                                                            <option value="0"
                                                                                {{ $combo['adults'] == 0 ? 'selected' : '' }}>
                                                                                0</option>
                                                                            <option value="1"
                                                                                {{ $combo['adults'] == 1 ? 'selected' : '' }}>
                                                                                1</option>
                                                                            <option value="2"
                                                                                {{ $combo['adults'] == 2 ? 'selected' : '' }}>
                                                                                2</option>
                                                                            <option value="3"
                                                                                {{ $combo['adults'] == 3 ? 'selected' : '' }}>
                                                                                3</option>
                                                                            <option value="4"
                                                                                {{ $combo['adults'] == 4 ? 'selected' : '' }}>
                                                                                4</option>
                                                                            <option value="5"
                                                                                {{ $combo['adults'] == 5 ? 'selected' : '' }}>
                                                                                5</option>
                                                                            <option value="6"
                                                                                {{ $combo['adults'] == 6 ? 'selected' : '' }}>
                                                                                6</option>
                                                                            <option value="7"
                                                                                {{ $combo['adults'] == 7 ? 'selected' : '' }}>
                                                                                7</option>
                                                                            <option value="8"
                                                                                {{ $combo['adults'] == 8 ? 'selected' : '' }}>
                                                                                8</option>
                                                                            <option value="9"
                                                                                {{ $combo['adults'] == 9 ? 'selected' : '' }}>
                                                                                9</option>
                                                                            <option value="10"
                                                                                {{ $combo['adults'] == 10 ? 'selected' : '' }}>
                                                                                10</option>
                                                                            <option value="11"
                                                                                {{ $combo['adults'] == 11 ? 'selected' : '' }}>
                                                                                11</option>
                                                                            <option value="12"
                                                                                {{ $combo['adults'] == 12 ? 'selected' : '' }}>
                                                                                12</option>
                                                                            <option value="13"
                                                                                {{ $combo['adults'] == 13 ? 'selected' : '' }}>
                                                                                13</option>
                                                                            <option value="14"
                                                                                {{ $combo['adults'] == 14 ? 'selected' : '' }}>
                                                                                14</option>
                                                                            <option value="15"
                                                                                {{ $combo['adults'] == 15 ? 'selected' : '' }}>
                                                                                15</option>
                                                                        </select>
                                                                    </div>
                                                                    ...
                                                                    <div class="form-group" style="display: none">
                                                                        ...
                                                                        <select id="numChildren" class="form-select"
                                                                            ...
                                                                            <option value="0"
                                                                                {{ $combo['children'] == 0 ? 'selected' : '' }}>
                                                                                0</option>
                                                                            <option value="1"
                                                                                {{ $combo['children'] == 1 ? 'selected' : '' }}>
                                                                                1</option>
                                                                            <option value="2"
                                                                                {{ $combo['children'] == 2 ? 'selected' : '' }}>
                                                                                2</option>
                                                                            <option value="3"
                                                                                {{ $combo['children'] == 3 ? 'selected' : '' }}>
                                                                                3</option>
                                                                            <option value="4"
                                                                                {{ $combo['children'] == 4 ? 'selected' : '' }}>
                                                                                4</option>
                                                                            <option value="5"
                                                                                {{ $combo['children'] == 5 ? 'selected' : '' }}>
                                                                                5</option>
                                                                            <option value="6"
                                                                                {{ $combo['children'] == 6 ? 'selected' : '' }}>
                                                                                6</option>
                                                                            <option value="7"
                                                                                {{ $combo['children'] == 7 ? 'selected' : '' }}>
                                                                                7</option>
                                                                            <option value="8"
                                                                                {{ $combo['children'] == 8 ? 'selected' : '' }}>
                                                                                8</option>
                                                                            <option value="9"
                                                                                {{ $combo['children'] == 9 ? 'selected' : '' }}>
                                                                                9</option>
                                                                            <option value="10"
                                                                                {{ $combo['children'] == 10 ? 'selected' : '' }}>
                                                                                10</option>
                                                                            <option value="11"
                                                                                {{ $combo['children'] == 11 ? 'selected' : '' }}>
                                                                                11</option>
                                                                            <option value="12"
                                                                                {{ $combo['children'] == 12 ? 'selected' : '' }}>
                                                                                12</option>
                                                                            <option value="13"
                                                                                {{ $combo['children'] == 13 ? 'selected' : '' }}>
                                                                                13</option>
                                                                            <option value="14"
                                                                                {{ $combo['children'] == 14 ? 'selected' : '' }}>
                                                                                14</option>
                                                                            <option value="15"
                                                                                {{ $combo['children'] == 15 ? 'selected' : '' }}>
                                                                                15</option>
                                                                        </select>
                                                                    </div>
                                                                    ...
                                                                    <div class="form-group" style="display: none">
                                                                        ...
                                                                        <select id="numInfants" class="form-select"
                                                                            ...
                                                                            <option value="0"
                                                                                {{ $combo['infants'] == 0 ? 'selected' : '' }}>
                                                                                0</option>
                                                                            <option value="1"
                                                                                {{ $combo['infants'] == 1 ? 'selected' : '' }}>
                                                                                1</option>
                                                                            <option value="2"
                                                                                {{ $combo['infants'] == 2 ? 'selected' : '' }}>
                                                                                2</option>
                                                                            <option value="3"
                                                                                {{ $combo['infants'] == 3 ? 'selected' : '' }}>
                                                                                3</option>
                                                                            <option value="4"
                                                                                {{ $combo['infants'] == 4 ? 'selected' : '' }}>
                                                                                4</option>
                                                                            <option value="5"
                                                                                {{ $combo['infants'] == 5 ? 'selected' : '' }}>
                                                                                5</option>
                                                                            <option value="6"
                                                                                {{ $combo['infants'] == 6 ? 'selected' : '' }}>
                                                                                6</option>
                                                                            <option value="7"
                                                                                {{ $combo['infants'] == 7 ? 'selected' : '' }}>
                                                                                7</option>
                                                                            <option value="8"
                                                                                {{ $combo['infants'] == 8 ? 'selected' : '' }}>
                                                                                8</option>
                                                                            <option value="9"
                                                                                {{ $combo['infants'] == 9 ? 'selected' : '' }}>
                                                                                9</option>
                                                                            <option value="10"
                                                                                {{ $combo['infants'] == 10 ? 'selected' : '' }}>
                                                                                10</option>
                                                                            <option value="11"
                                                                                {{ $combo['infants'] == 11 ? 'selected' : '' }}>
                                                                                11</option>
                                                                            <option value="12"
                                                                                {{ $combo['infants'] == 12 ? 'selected' : '' }}>
                                                                                12</option>
                                                                            <option value="13"
                                                                                {{ $combo['infants'] == 13 ? 'selected' : '' }}>
                                                                                13</option>
                                                                            <option value="14"
                                                                                {{ $combo['infants'] == 14 ? 'selected' : '' }}>
                                                                                14</option>
                                                                            <option value="15"
                                                                                {{ $combo['infants'] == 15 ? 'selected' : '' }}>
                                                                                15</option>
                                                                        </select>
                                                                    </div>
                                    ...
                                    <div class="booking-review-content">
                                        <div class="card mb-3">
                                            ...
                                            <div class="card-header" style="text-align: center;">
                                                {{ $combo['departureLocation'] ?? '' }} &rarr;
                                                <span class='font_manish';> {{ $combo['triptimeReq'] ?? '' }}
                                                </span>
                                                {{ $combo['city_aa'] ?? '' }}({{ $combo['city_code'] ?? '' }})
                                            </div>
                                        </div>
                                        @foreach ($combo['segment2'] as $seg)
                                            @php
                                            ...
                                            @if (isset($departureCode) && $departureCode == $combo['to'])
                                                <div class="card mb-3">
                                                    ...
                                                    <div class="card-header" style="text-align: center;">
                                                        {{ $combo['city_aa'] ?? '' }}({{ $combo['city_code'] ?? '' }})
                                                        &rarr;<span
                                                            class='font_manish';>{{ $combo['triptimeReturn'] ?? '' }}</span>
                                                        {{ $combo['departureLocation'] ?? '' }}
                                                       
                                                    </div>
                                                </div>
                                                <div class="review-section">

                                                    <div class="service-info row m-1">
                                                            ...
                                                            <div class="flex-self-start">
                                                                ...
                                                                <div class="text-lh-sm ml-1">
                                                                    ...
                                                                    <h6
                                                                        ...
                                                                        {{ $arrivalDate }}<br>
                                                                        {{ $arrivalTime }}</h6> <span
                                                                        class="font-size-14 font-weight-normal text-gray-1">{{ $arrivalName }},
                                                                        {{ $arrivalCity }}
                                                                        ({{ $arrivalCityCode }})
                                                                        ,{{ $arrivalCountry }}</span>
                                        ...
                                        <div class="review-section total-review">
                                            <ul class="review-list m-3">
                                                <li class="final-total d-block">
                                                    <div class="d-flex justify-content-between">
                                                        <div class="label">BaseFare:</div>
                                                        <div class="val">
                                                            <span class="font_manish">
                                                                <?= $icon ?>{{ number_format($combo['totalBaseFares'], 2) }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </li>
                                                <li class="final-total d-block">
                                                    <div class="d-flex justify-content-between">
                                                        ...
                                                        <div class="val">
                                                            <span class="font_manish">
                                                                <?= $icon ?>{{ number_format($combo['totalTaxess'], 2) }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </li>
                                                <li class="final-total d-block">
                                                    <div class="d-flex justify-content-between">
                                                        ...
                                                        <div class="val">
                                                            <span class="font_manish">
                                                                <?= $icon ?>{{ number_format($combo['totalDiscounts'], 2) }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </li>
                                                ...
                                                <li class="final-total d-block">
                                                    <div class="d-flex justify-content-between">
                                                        ...
                                                        <div class="val">
                                                            <span class="font_manish">
                                                                <?= $icon ?>{{ number_format($combo['totalFare'], 2) }}
                                                            </span>
                                                        </div>
                                                    </div>
                                                </li>
