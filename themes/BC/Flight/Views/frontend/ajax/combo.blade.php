...
<div class="flight-card mx-auto manishbhai">
   <div class="flight-card_SHOW" data-details="details-<?= $a ?>">
      <div class="flight-info">
         ...
         <div class="text-right">
            <div class="price " id="price-<?= $a ?>"><span class="font_manish"><?=$icon ?>{{ number_format($totalFare, 2) }}</span></div>
            <br>
         </div>
         <form action="{{ route('booking.checkout.post', ['code' => $msg]) }}" method="POST">
            @method('POST')
            @csrf
   ...
   <div class="container-fluid" id="details-<?= $a ?>" style="display: none; border: 2px solid  #ff7d01;margin-top: 0.5rem; border-radius: 9px;">
      <div class="row pt-2 rowManish">
         ...
         {{-- return pack booking deatils end --}}
      </div>
   </div>
</div>
<?php
...
<script>
   ...
   priceElement.innerHTML = getCurrencyIcon('{{$curr_currency}}');
   }
   });
</script>
