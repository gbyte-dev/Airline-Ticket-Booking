...
document.querySelectorAll('.form-control').forEach(input => {
    input.addEventListener('input', function () {
        let formData = new FormData(document.getElementById('form-checkout'));

        // Send an AJAX request to auto-save form data
        axios.post('/auto.save.form', formData)
            .then(response => {
                console.log('Form auto-saved successfully!');
                
                // Store the form ID in case it's newly generated
                document.querySelector('input[name="form_id"]').value = response.data.form_id;
            })
            .catch(error => {
...
$(document).ready(function () {
    ...
    $('.form-control').on('input', function () {
        ...
        let formData = new FormData($('#form-checkout')[0]);

        // Send an AJAX request to auto-save form data
        $.ajax({
            url: 'auto.save.form', // URL to send the request to
            ...
            success: function (response) {
                ...
                // Store the form ID in case it's newly generated
                $('input[name="form_id"]').val(response.form_id);
            },
