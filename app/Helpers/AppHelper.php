...
function convertCurrency($amount, $currency, $conversionRates = ['INR' => 1, 'USD' => 0.012, 'CAD' => 0.016])
    {
        return isset($conversionRates[$currency]) ? $amount * $conversionRates[$currency] : $amount;
    }

function normalizeTime($hours, $minutes) {
    // Convert minutes to hours if they are 60 or more
    if ($minutes >= 60) {
...
function generate_css($model){
 ...
 function getcontrys($id = false)
    {
        if (!$id) {
          
            return DB::table('bravo_return_conutry')->get();
        } else {
            
            return DB::table('bravo_return_conutry')->where('id', $id)->first();
        }
    }
function getCountriesWithCities($id=1) {
    
    // Join the `bravo_return_conutry` table with `cities` based on the country_id
    $city_detail = DB::table('bravo_return_filght_city')
    ->where('country_id',$id)->where('display', 'display_yes')
        ->get();

    // Return the data to the view or process it further
    return $city_detail;
}
...
function addDaysToDate($inputDate, $daysToAdd) {
    ...
    // Create a DateTime object with the given input date and current year
    $date = DateTime::createFromFormat('M d Y', "$inputDate $currentYear");

    // Check if the date is valid
    if (!$date) {
        echo "Invalid date format. Please use 'MMM DD' format like 'Nov 18'.";
        return null;
    }
    ...
    // Format the output date to 'M d' format
