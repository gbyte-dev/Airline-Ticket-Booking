...
class FlightController extends Controller
{
    ...
    public function index(Request $request)
    {
		...
		$currency = $_COOKIE['currency']??'CAD';
        $data['currency'] = $selectedCurrency = $request->query('currency', $currency);
        ...
        //  redirect if fromwhere and from to same 
        if ($flightRequestData['fromWhere'] == $flightRequestData['toWhere']) {
            $data['error_message'] = "Departure and Destination can't be the same.";
        ...
        if (!empty($flightRequestData['returnDate'])) {
            ...
            } elseif (isset($apiResponse['searchResult']['tripInfos']['COMBO'])) {
                $apiResponse['searchResult']['tripInfos']['RETURN'] = $this->processFlights($apiResponse['searchResult']['tripInfos']['COMBO'], $selectedCurrency);
            } else {
    ...
    private function filterAndSortFlights(&$apiResponse, Request $request, $selectedCurrency, $hasReturn)
{
    ...
    if (isset($apiResponse['searchResult']['tripInfos']['ONWARD'])) {
        ...
        // Sort onward flights
        $apiResponse['searchResult']['tripInfos']['ONWARD'] = $this->sortFlights(
            $apiResponse['searchResult']['tripInfos']['ONWARD'],
            $sortOrder,
            $selectedCurrency
        );
    ...
    // Filter combo flights regardless of return presence
    if (isset($apiResponse['searchResult']['tripInfos']['COMBO'])) {
        $apiResponse['searchResult']['tripInfos']['COMBO'] = $this->filterFlights(
            $apiResponse['searchResult']['tripInfos']['COMBO'],
            $filterStops,
            $filterTimes,
            $filterClasses,
            $filterAirlines
        ...
        // Sort combo flights
        $apiResponse['searchResult']['tripInfos']['COMBO'] = $this->sortFlights(
            $apiResponse['searchResult']['tripInfos']['COMBO'],
            $sortOrder,
            $selectedCurrency
        );
    ...
    private function filterFlights($flights, $filterStops, $filterTimes, $filterClasses, $filterAirlines)
    {
        return array_filter($flights, function ($flight) use ($filterStops, $filterTimes, $filterClasses, $filterAirlines) {
            foreach ($flight['sI'] as $segment) {
                $numStops = count($flight['sI']) - 1;
                if (!empty($filterStops)) {
                    $numStopsStr = (string)$numStops;
                    if (!in_array($numStopsStr, $filterStops)) {
                        return false; 
                    }
                }
    ...
    private function captureSearchInputs(Request $request)
    {
		...
		$paxInfo = [
			'ADULT' => isset($seatTypeArray[0]) ? (int)$seatTypeArray[0] : 0,
			'CHILD' => isset($seatTypeArray[1]) ? (int)$seatTypeArray[1] : 0,
			'INFANT' => isset($seatTypeArray[2]) ? (int)$seatTypeArray[2] : 0
		];

		// Ensure at least one adult is set to 1 when all are zero
		if ($paxInfo['ADULT'] == 0 && $paxInfo['CHILD'] == 0 && $paxInfo['INFANT'] == 0) {
			$paxInfo['ADULT'] = 1;
		}
    ...
    {
         //$apiKey = env('FLIGHTAPIKEY'); // Replace with your API key
         $apiurl = env('APIURL'); // Replace with your API url
$apiKey = '1108638608b81018-36ac-448d-b318-953cc1afb894'; 
         // Initialize cURL session
$curl = curl_init();

// Static search data
$travelDate = '2025-04-20';
$selectedCabin = 'ECONOMY'; // options: ECONOMY, PREMIUM_ECONOMY, BUSINESS, FIRST, or "any"

// Define the cabin classes
...
if ($fromCode === $toCode) {
    die("From and To cannot be same");
}

// Define the static request data
$requestData = [
    'searchQuery' => [
...
$data = json_decode($response, true);
    ...
    private function convertCurrency($amount, $currency, $conversionRates = ['INR' => 1, 'USD' => 0.012, 'CAD' => 0.016])
    {
	    ...
	    public function referAndEarn(Request $request)
    {
        ...
        return view('Flight::frontend.referandearn', $data);
    }
