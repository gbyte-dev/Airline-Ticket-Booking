jQuery(function ($) {
    $.fn.bravoAutocomplete = function (options) {
        ...
        return this.each(function () {
            ...
            // Create and append the loader with CSS
            var loader = main.find('.input-loader');
    ...
    for (var index in options.dataDefault) {
        var item = options.dataDefault[index];
        // Construct the title with additional information
        var displayText = item.title + ' - ' + item.airport_name + ', ' + item.city + ', ' + item.airport_country_name + ' (' + item.airport_country_code + ')';
        
        items += '<div class="item" data-id="' + item[key] + '" data-text="' + displayText + '"> <i class="' + options.iconItem + '"></i> ' + displayText + ' </div>';
    }
    main.find('.bravo-autocomplete .list-item').html(items);
    main.find('.bravo-autocomplete').removeClass("on-message");
} else {
                            ...
                            success: function (res) {
                                ...
                                if (res.status === 1) {
                                    ...
                                    for (var ix in res.data) {
                                        ...
                                        items += '<div class="item" data-id="' + item[key] + '" data-text="' + displayText + '"> <i class="' + options.iconItem + '"></i> ' + get_highlight(displayText, query) + (item.desc ? '<span class="item-desc">'+item.desc+'</span>' : '') +' </div>';
                                    }                                    
                                    main.find('.bravo-autocomplete .list-item').html(items);
                                    main.find('.bravo-autocomplete').removeClass("on-message");
                                }
